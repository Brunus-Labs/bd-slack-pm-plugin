...
const SettingsContainer = ({name, children, onReset}) => (React.createElement(Form.FormSection, null,
    ...
    React.createElement(Flex, {justify: Flex.Justify.END},
        React.createElement(Button, {
            size: Button.Sizes.SMALL, onClick: () => confirm(name, "Reset all settings?", {
                onConfirm: () => onReset()
            })
        }, "Reset"))));
...
const NumberInput = ({value, min, max, fallback, onChange}) => {
    ...
    return (React.createElement("div", {className: "container-BetterVolume"},
        React.createElement("input", {
            type: "number", className: "input-BetterVolume", min: min, max: max, value: !isEmpty ? Math.round((value + Number.EPSILON) * 100) / 100 : "", onChange: ({target}) => {
